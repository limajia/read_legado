...
object ReadBook : CoroutineScope by MainScope() {
    ...
    var bookSource: BookSource? = null//切换数据后，更新数据源
    var msg: String? = null
    ...
    fun contentLoadFinish(
        ...
        Coroutine.async {
            val contentProcessor: ContentProcessor = ContentProcessor.get(book.name, book.origin)
            val displayTitle = chapter.getDisplayTitle(
                contentProcessor.getTitleReplaceRules(),
                book.getUseReplaceRule())
            //第19章 正经年轻人不吃早饭
            println("malijia:contentLoadFinish:displayTitle:=$displayTitle")
            //得到章节按符号拆分的集合内容
            val contents: BookContent = contentProcessor.getContent(book, chapter, content, includeTitle = false)
            println("malijia:contentLoadFinish:contents:=${contents.textList.forEach(::println)}")
            val textChapter = ChapterProvider.getTextChapter(book, chapter, displayTitle, contents, chapterSize)
            when (val offset = chapter.index - durChapterIndex) {
                0 -> {
                    ...
                    callBack?.upMenuView()
                    curPageChanged()
                    callBack?.contentLoadFinish()//???为什么再一次这个？？？？
                }
