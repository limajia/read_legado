...
object ChapterProvider {
    ...
    /**
     * 获取拆分完的章节数据（拆分章节数据，重新计算页数等）
     */
    suspend fun getTextChapter(
        book: Book,//书籍信息
        bookChapter: BookChapter,//章节信息
        displayTitle: String,//章节title
        ...
        if (ReadBookConfig.titleMode != 2 || bookChapter.isVolume) {
            ...
            displayTitle.splitNotBlank("\n").forEach { text ->
                ...
                setTypeText(
                    ...
                    titlePaint,//标题画笔
                    titlePaintTextHeight,//标题文字高度
                    titlePaintFontMetrics,//标题字体度量
                    isTitle = true,//是否是标题
                    emptyContent = contents.isEmpty(),//是否是没有正文
                    isVolumeTitle = bookChapter.isVolume//是否是卷标题
                ).let {
                    //返回的是下一行x轴起始位置和y轴起始位置
                    absStartX = it.first
                    durY = it.second
                }
            }
        ...
        textPages.last().height = durY + 20.dpToPx()//最后一页的高度，添加一个空白的间距
        textPages.last().text = stringBuilder.toString() //猜测是：最后一页的文本
        textPages.forEachIndexed { index, textPage: TextPage ->
            textPage.index = index
            textPage.pageSize = textPages.size
            textPage.chapterIndex = bookChapter.index
            textPage.chapterSize = chapterSize
    ...
    private suspend fun setTypeText(
        ...
        for (lineIndex in 0 until layout.lineCount) {
            ...
            if (textLine.isParagraphEnd) {//段落结束,添加换行
                stringBuilder.append("\n")
            }
            //段落数一直在+1，找到最后一个TextPage的最后一个TextLine的paragraphNum。如果没有就是1
