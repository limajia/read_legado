...
class ReadBookActivity : BaseReadBookActivity(),
    ...
    private val tocActivity: ActivityResultLauncher<String> = registerForActivityResult(TocActivityResult()) {
            it?.let {
    ...
    private var loadStates: Boolean = false//用来加载语音信息
    ...
    override fun onMenuItemSelected(itemId: Int): Boolean {
        when (itemId) {
            R.id.menu_aloud -> when (AppConfig.contentSelectSpeakMod) {// 1:切换为从选择的地方开始一直朗读 0:切换为朗读选择的内容
